...
if med == "light":
    mphiratios = [1.0e-10, 1.0e-9, 1.0e-7, 1.0e-6, 1.0e-5, 1.0e-4, 1.0e-3, 1.0e-2]
    # mphiratios = [1.e-4, 1.e-3]
elif med == "fixed_light":
    mphiratios = (1.0 * u.eV).to(u.MeV).value
else:
    mphiratios = 1.0


def get_lim(ex, mphi_ratio=mphiratios, medtype="light", phase=phase):
    if phase:
        print(
            f"{datetime.datetime.now()}: Getting {medtype} mediator phase limits for {ex}"
        )
    else:
        print(
            f"{datetime.datetime.now()}: Getting {medtype} mediator decoherence limits for {ex}"
        )
    ## Define exp. w/ qmin
    class_ = getattr(x, ex)
    exp = class_()
    ...
    if (phase) and (med == "light"):
        lims = [
            [
                cs_limit(mx, ex=exp, medtype=medtype, mphi=mpr * mx, phase=phase)
                for mx in mxs
            ]
            for mpr in mphi_ratio
        ]
        ...
        lims = [
            cs_limit(mx, ex=exp, medtype="light", mphi=mphiratios * u.MeV, phase=phase)
            for mx in mxs
        ]
    elif (~phase) and (med == "fixed_light"):
        lims = cs_limit(
            mxs, ex=exp, medtype="light", mphi=mphiratios * u.MeV, phase=phase
        )
    else:
        try:
            lims = [
                cs_limit(mxs, ex=exp, medtype=medtype, mphi=mpr * mxs, phase=phase)
                for mpr in mphi_ratio
            ]
        except TypeError:
            lims = cs_limit(
                mxs, ex=exp, medtype=medtype, mphi=mphi_ratio * mxs, phase=phase
            )
    ## Save to file.
    if medtype == "heavy":
