...
def phase_integral(q, mx, ex=gdm, vdm=vdm, ve=ve, vesc=vesc, real=True):
    ...
    vdm = vdm.value
    vesc = vesc.value
    prefac = -0.25j*np.sqrt(np.pi)*vdm/ve*np.exp(-q**2*ex.deltax.value *
                                                 (2j*ve+mx*vdm**2*ex.deltax.value)/(4*mx*ve**2))
...
def rate_integral(mx, ex=gdm, phase=False, exactphase=False):
    def formfacexp(q): return (1+ex.A*helmformfac(q, ex=ex)
                               ** 2+ex.N*(formfac(q*ex.r.value))**2)
    if phase:
        def integrand(q): return q*phase_integral(q,
                                                  mx.value, ex=ex)*formfacexp(q)
        if exactphase:
        ...
        if ex.name in ['GDM', 'BECCAL', 'Stanford']:
            ...
            def formfacexp(q): return (1+ex.N*(formfac(q*ex.r.value))**2)

        def integrand(q): return q*(1.-np.sin(q*ex.deltax.value) /
                                    (q*ex.deltax.value))*formfacexp(q)*expfac(q, mx)
...
def light_rate_integral(mx, ex=gdm, mphi=None, phase=False, exactphase=False):
    ...
    if ex.name in ['GDM', 'BECCAL', 'Stanford']:
        def formfacexp(q): return (1+ex.A*helmformfac(q, ex=ex)
                                   ** 2+ex.N*(np.exp(-(0.5*q*ex.r.value)**2))**2)
    ...
    if phase:
        def integrand(q): return q/(q**2 + mphi.value**2)**2 * \
            phase_integral(q, mx.value, ex=ex)*formfacexp(q)
        if exactphase:
            return quad_vec(integrand, 0., 1./ex.r.value+1.e10)[0]*u.MeV**(-2)
        else:
            # this is good to ~1% for GDM, but doesn't fit all other exp.
        ...
        def integrand(q): return q/(q**2 + mphi.value**2)**2*(1.-np.sin(q *
                                                                        ex.deltax.value)/(q*ex.deltax.value))*formfacexp(q)*expfac(q, mx)
