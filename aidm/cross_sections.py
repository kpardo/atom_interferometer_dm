...
def cs_limit(mx, ex=gdm, medtype='light',mphi=None, phase=False):
    ...
    if phase:
        return np.sqrt(ex.Nind)/np.sqrt(ex.Nmeas)*1./rate(mx, ex=ex, medtype=medtype, mphi=mphi, phase=phase)*u.cm**2
    else:
        s = rate(mx, ex=ex, medtype=medtype, mphi=mphi,
                 phase=phase)
        return np.sqrt(ex.Nind)/np.sqrt(ex.Nmeas)*1./s * u.cm**2
